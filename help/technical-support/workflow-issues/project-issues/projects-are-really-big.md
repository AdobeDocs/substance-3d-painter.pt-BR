...
Ao usar muitos recursos no projeto, o Substance 3D Painter os copia. Por exemplo, se você usou um alfa para tinta algo. Se, posteriormente, você excluir a camada quando o alfa for pintado, o Substance 3D Painter não removerá automaticamente o recurso.
...
### Reduzir o tamanho dos mapas de malha feitos bake

Em geral, o maior culpado e a razão pela qual um projeto ocupa tanto espaço no disco é porque os mapas de malha feitos bake são muitos e grandes.

Para reduzir o tamanho dos mapas de malha, algumas ações podem ser feitas:

* *Use uma resolução de fça bake mais baixa.*\
  Embora o Mapa normal possa se beneficiar ao ser feito bake em 4K, esse pode não ser o caso do mapa de posição, que geralmente é apenas sobre gradientes coloridos. Fça bake em dois passos em duas resoluções diferentes para misturar tamanhos de arquivo diferentes.
* *Exporte as texturas e reduza manualmente o impacto.*\
  Por padrão, o Substance 3D Painter faz bake todas as texturas como imagens RGBA em 16 bits, o que inclui baker em tons de cinza, como a Oclusão de ambiente.

  Para reduzir as texturas de faço bake para impressão, use este passo a passo:
  1. Desative a configuração “Aplicar difusão” na janela do Baker
  1. Defina “Dilatação com” como um valor razoável (32 pixels para uma resolução de 2048, por exemplo)
  1. Fça bake todas as Texturas na mesma resolução
  1. Exporte as texturas feitas bake com a predefinição de exportação “Mapas de malha” como PNG de 16 bits com o preenchimento definido como “Sem preenchimento (passagem)”
...
É importante que os mapas de malha permaneçam pelo menos texturas de 16 bits. Embora o textura de 8 bits possa ter um espaço menor, ele introduzirá artefatos em Materiais inteligentes e Geradores de máscara. Recomendamos o PNG porque ele é um formato de compactação sem perdas, o que significa que ainda compactará o textura sem introduzir artefatos e também é compatível com 16 bits.
