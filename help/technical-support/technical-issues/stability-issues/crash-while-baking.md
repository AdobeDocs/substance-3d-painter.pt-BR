...
O Substance 3D Painter pode travar durante o processo de fça bake em algumas configurações. Esta página agrupa uma lista de problemas conhecidos e como mitigá-los.

## Falha ao Fazer bake visualização

Por padrão, o Substance 3D Painter exibe no visor o estado em andamento do fça bake de uma textura. Em alguns computadores, esse recurso pode causar instabilidades.

Para desativá-la:

1. Use **Editar > Configurações** para abrir as configurações principais
1. Em **Geral**, rola para baixo até a seção chamada **Opções de Fça bake**.
1. Desmarque/Desabilite a opção **Habilitar processo de fça bake de visualização ao vivo** .

## Falha com Rastreamento de raios do GPU

Em algumas GPUs com drivers instáveis, o processo de fça bake pode levar a falhas devido ao recurso de Rastreamento de raios do GPU.
...
O aplicativo pode travar durante o processo de fça bake em alguma configuração de computador em execução com uma CPU Ryzen. Em geral, uma atualização do BIOS corrige o problema.
...
Por padrão, ao fazer bake, as malhas de alto polígono são pré-processadas em arquivos **\*.assbin** para acelerar o retorno mais tarde. Em alguns casos raros, esses arquivos podem travar o aplicativo se tiverem sido gerados com uma versão diferente. Simplesmente excluí-los deve resolver o problema, pois eles serão regenerados.
