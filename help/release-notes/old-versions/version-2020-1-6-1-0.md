---
helpx_url: "https://helpx.adobe.com/br/substance-3d-painter/release-notes/old-versions/version-2020-1-6-1-0.html"
breadcrumb-title: ''
...
O **Substance Painter 2020.1 (6.1.0)** oferece um novo exportador, scripts python, um novo baker de curvatura, novo conteúdo e muitas outras melhorias de fluxo de trabalho.
...
* **Novas Configurações Globais**\
  ...
  * **Exportar parâmetros de sombreadores**: se habilitado, exporta um arquivo json armazenando as configurações de sombreador e suas atribuições de Conjunto de Texturas.
...
* **Salvar configurações sem exportar**\
  Agora é possível salvar a configuração de exportação do projeto sem ter que exportar texturas usando o novo botão **Salvar configurações**. Isso é útil quando você deseja ajustar um projeto sem gerar novamente texturas, o que pode levar algum tempo.

  ![](../../assets/save-settings.png)
* **Clique e arraste para habilitar/desabilitar exportações rapidamente**\
  Use o clique do mouse e arraste para ativar ou desativar rapidamente os Conjuntos de texturas e texturas:
...
* <b>Criar predefinições de exportação</b>\
  A guia <b>Modelo de saída</b> é idêntica à guia <b>Configuração</b> do exportador antigo. Ele pode ser usado para criar predefinições de exportação para nomear e empacotar texturas rapidamente. Para saber como criar predefinições de exportação personalizadas, confira nossa página de documentação.
* **Configurar formato de arquivo e profundidade de bits em predefinições de exportação**\
  Uma nova adição às predefinições de exportação é a possibilidade de definir o formato do arquivo e a profundidade de bits por textura para facilitar ainda mais o compartilhamento de uma configuração entre projetos.
...
Essa guia exibe o processo de exportação em andamento, bem como um resumo global de toda a textura exportada.
...
Nesta versão, o baker de curvatura foi aprimorado e o baker de Oclusão de ambiente recebeu alguns novos parâmetros.

* <b>Nova curvatura do padeiro de malha</b>\
  O antigo baker de curvatura agora está obsoleto e foi substituído pela nova versão “de malha”, que foi recentemente adicionada ao Substance Designer. Para saber mais sobre as novas configurações do padeiro, confira a página de documentação.\
  ...
  * <b>Contato de malha</b>: a interpenetração entre malhas agora produz informações de curvatura (o que não era o caso com o baker anterior).
  * <b>Malha de alto polígono</b>: os detalhes agora são feitos bake diretamente da malha de alto polígono em vez de serem convertidos do mapa normal.
  * <b>Desempenho</b>: o novo baker usa o Rastreamento de raios para calcular detalhes, o que significa que ele se beneficia da aceleração do Rastreamento de raios do GPU (RTX/Optix).

  ![](../../assets/baker-comparison-curvature.jpg){width="500px"}

  Ainda é possível acessar o antigo baker de curvatura por motivos de compatibilidade nas configurações do baker de curvatura. Escolha a configuração **Gerar do Mapa normal** para usar o baker antigo.

  ![](../../assets/curvature-baker-setting.png)
* <b>Oclusão de ambiente aprimorada</b>\
  O baker de Oclusão de ambiente foi aprimorado para suportar novas configurações:

  * <b>Plano terrestre</b>: simula um plano abaixo da malha para criar sombreamento proveniente do solo. Para obter mais informações, consulte a documentação de Oclusão de ambiente.
  * <b>Ignorar face de fundo</b>: há um novo sufixo de nome de malha para ignorar um objeto específico ao fazer bake a Oclusão de ambiente. Por exemplo, permite ignorar detalhes de geometria flutuante. Para obter mais informações, consulte a documentação Correspondência por nome
...
* [Baker] Falha ao fazer bake com optix quando não há poli alto
