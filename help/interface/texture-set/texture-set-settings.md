---
helpx_url: "https://helpx.adobe.com/br/substance-3d-painter/interface/texture-set/texture-set-settings.html"
breadcrumb-title: ''
...
| **Descrição** | Campo de texto que permite adicionar informações sobre um conjunto de texturas. Este texto é exibido na lista [Conjunto de Texturas](texture-set-list.md) e nas janelas [Fazendo bake](../../baking/baking.md). |
...
| **Instância de sombreamento** | Defina o [Sombreador](../shader-settings/shader-settings.md) a ser usado para renderizar o Conjunto de Texturas especificado no [visor](../viewport/viewport.md). |
...
Essas configurações controlam vários comportamentos em como os canais são gerados, notavelmente como os canais são combinados com as texturas feitas bake (mapas de malha).

| Configuração | Descrição |
| --- | --- |
| **Mistura normal** | Controla como o “mapa normal feito bake” deve ser combinado com o canal “Normal”. Os valores possíveis são:<ul data-preserve-html="true"><li data-preserve-html="true"><strong> Substituir </strong>: Ignorar o “mapa normal feito bake” e usar apenas o canal “Normal” para este Conjunto de Texturas. Pode ser usado para tinta sobre um mapa normal feito bake. Consulte a documentação da [pintura de canal avançada](../../painting/advanced-channel-painting/normal-map-painting.md)para obter mais informações. se o canal Normal não estiver presente ou se a saída do canal Normal estiver vazia, o mapa normal feito bake ainda será usado.</li><li data-preserve-html="true"><strong> Combinar </strong> (padrão): use uma função orientada a detalhes para combinar o canal “Normal” e o “mapa normal feito bake”.</li></ul>  **Observação:** esta configuração poderá ser desabilitada se o canal estiver ausente na lista de canais. Se o canal estiver ausente, o valor de mixagem padrão será usado. |
| **Height para o método normal** | Controla o método a ser usado para converter o canal de height em um mapa normal. Os valores possíveis são:<ul data-preserve-html="true"><li data-preserve-html="true"><strong>Nítido</strong>: produz um mapa normal mais definido, sob o risco de inserir ruído e suavização. Adaptado para padrões repetidos, como tecidos.</li><li data-preserve-html="true"><strong>Suavizar (Sobel)</strong> (padrão): produza um mapa normal mais suave com um filtro Sobel, sob o risco de perder detalhes. Adaptado para a maioria dos casos.</li></ul> |
| **Mistura de Oclusões de ambiente** | Controla como a “oclusão de ambiente feita bake” deve ser combinada com o canal “Oclusão de ambiente”. Os valores possíveis são:<ul data-preserve-html="true"><li data-preserve-html="true"><strong> Substituir </strong>: Ignorar a “oclusão de ambiente feita bake” e usar apenas o canal “Oclusão de ambiente” para este Conjunto de Texturas. Pode ser usado para tinta sobre uma oclusão de ambiente feita bake. Consulte a documentação da [pintura de canal avançada](../../painting/advanced-channel-painting/ambient-occlusion-painting.md) para obter mais informações.  </li><li data-preserve-html="true"><strong> Multiplicação </strong> (padrão): use uma operação de multiplicação para combinar o canal “Oclusão de ambiente” e a “oclusão de ambiente feita bake”.  </li></ul>  **Observação:** esta configuração poderá ser desabilitada se o canal estiver ausente na lista de canais. Se o canal estiver ausente, o valor de mixagem padrão será usado. |
...
Os mapas de malha são texturas feitas bake específicas para a malha e o conjunto de textura usados para aumentar a qualidade da texturização com a ajuda de filtros, Materiais inteligentes e Máscaras inteligentes. Para obter mais detalhes, consulte a documentação de [fazendo bake](../../baking/baking.md).
